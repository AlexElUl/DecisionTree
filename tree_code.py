...
def find_best_split(feature_vector, target_vector):
    ...
    * :math:`p_1` и :math:`p_0` — доля объектов класса 1 и 0 соответственно.
    ...
    idx = np.argsort(x)
    x_sort, y_sort = x[idx], y[idx]
    thresholds = (x_sort[:-1] + x_sort[1:]) / 2
    ginis = []
    best_gini = float('inf')
    best_thresh = None 
    ...
    for i, t in enumerate(thresholds):
        ...
        gini = (len(left)*gini_l + len(right)*gini_r) / len(y_sort)
        ginis.append(gini)
        
        if gini < best_gini or (gini == best_gini and t < best_thresh):
            best_gini = gini
            best_thresh = t 

    return np.array(thresholds), np.array(ginis), best_thresh, best_gini
